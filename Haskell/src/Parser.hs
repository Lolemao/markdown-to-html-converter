...
-- parses zero or more p until e is satisfied
manyTill :: Parser a -> Parser b -> Parser [a]
manyTill p e = e *> pure [] <|> (:) <$> p <*> manyTill p e

-- parses one or more p until e is satisfied
someTill :: Parser a -> Parser b -> Parser [a]
someTill p e = (:) <$> p <*> manyTill p e

-- Lookahead to see if the parser will succeed
lookAhead :: Parser a -> Parser a
lookAhead p = Parser f
  where
    f x = case parse p x of
      Result _ v -> Result x v
      Error e    -> Error e

-- removes trailing spaces
endSpaces :: Parser String
endSpaces = Parser (\x -> Result (dropWhileEnd isSpace x) "")

-- removes trailing inline spaces
endInlineSpaces :: Parser String
endInlineSpaces = Parser (\x -> Result (dropWhileEnd (`elem` "\t\r\f\v ") x) "")

-- is a positive integer
isPosInt :: Parser Char
isPosInt = satisfy (\x -> isDigit x && x /= '0')

-- one or more inline spaces
someInlineSpace :: Parser String
someInlineSpace = some (oneof "\t\r\f\v ")